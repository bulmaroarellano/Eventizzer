...
async function list(filter) {
  const response = await axios.get(
    `/events?page=${filter.page}&limit=${filter.limit}&events=${filter.events ? filter.events : ''}`,
  );
  return response.data;
}

async function filterEvents(request, filter) {
  const response = await axios.get(`/events?page=${filter.page}&limit=${filter.limit}${request}`);
  return response.data;
}
