...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/administrator/autocomplete`, { params });
  return response.data;
}

const AdministratorSelectItem = (props) => {
  ...
  const mapper = {
    intoSelect(originalValue) {
      ...
      let label = originalValue.label ? originalValue.label : originalValue.role;
