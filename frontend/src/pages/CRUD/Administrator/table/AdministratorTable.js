...
import * as dataFormat from 'pages/CRUD/Administrator/table/AdministratorDataFormatters';

import actions from 'actions/administrator/administratorListActions';
...
const AdministratorTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Role', title: 'role'},{label: 'Name', title: 'name'},{label: 'LastName', title: 'lastName'},{label: 'UserName', title: 'userName'},{label: 'Password', title: 'password'},
  ...
  const loading = useSelector((store) => store.administrator.list.loading);
  const rows = useSelector((store) => store.administrator.list.rows);
  const count = useSelector((store) => store.administrator.list.count);
  const modalOpen = useSelector((store) => store.administrator.list.modalOpen);
  const idToDelete = useSelector((store) => store.administrator.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/administrator/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/administrator/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Administrator</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Administrator'
                    : 'http://localhost:8080/api-docs/#/Administrator'
            }
        >
            API documentation for administrator
        </a>
    </div>
    <br/>

<Link to="/admin/administrator/new">
  <button
