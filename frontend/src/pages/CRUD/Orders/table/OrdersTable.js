...
import * as dataFormat from 'pages/CRUD/Orders/table/OrdersDataFormatters';
...
const OrdersTable = () => {
  ...
  const count = useSelector((store) => store.orders.list.count);
  const modalOpen = useSelector((store) => store.orders.list.modalOpen);
  const idToDelete = useSelector((store) => store.orders.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/orders/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/orders/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
        ...
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Orders'
                    : 'http://localhost:8080/api-docs/#/Orders'
            }
        >
