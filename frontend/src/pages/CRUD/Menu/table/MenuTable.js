...
import * as dataFormat from 'pages/CRUD/Menu/table/MenuDataFormatters';

import actions from 'actions/menu/menuListActions';
...
const MenuTable = () => {
  ...
  const loading = useSelector((store) => store.menu.list.loading);
  const rows = useSelector((store) => store.menu.list.rows);
  const count = useSelector((store) => store.menu.list.count);
  const modalOpen = useSelector((store) => store.menu.list.modalOpen);
  const idToDelete = useSelector((store) => store.menu.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/menu/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/menu/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Menu</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Menu'
                    : 'http://localhost:8080/api-docs/#/Menu'
            }
        >
            API documentation for menu
        </a>
    </div>
    <br/>
