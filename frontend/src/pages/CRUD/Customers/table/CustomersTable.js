...
import * as dataFormat from 'pages/CRUD/Customers/table/CustomersDataFormatters';

import actions from 'actions/customers/customersListActions';
...
const CustomersTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Name', title: 'name'},{label: 'LastName', title: 'lastName'},{label: 'Email', title: 'email'},{label: 'Orders', title: 'orders'},{label: 'Address1', title: 'address1'},{label: 'Address2', title: 'address2'},{label: 'City', title: 'city'},{label: 'State', title: 'state'},{label: 'Lat', title: 'lat'},{label: 'Long', title: 'long'},
  ...
  const loading = useSelector((store) => store.customers.list.loading);
  const rows = useSelector((store) => store.customers.list.rows);
  const count = useSelector((store) => store.customers.list.count);
  const modalOpen = useSelector((store) => store.customers.list.modalOpen);
  const idToDelete = useSelector((store) => store.customers.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/customers/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/customers/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Customers</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Customers'
