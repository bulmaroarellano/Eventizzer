...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/customers/autocomplete`, { params });
  return response.data;
}

const CustomersSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.customers.hasPermissionToCreate
});
