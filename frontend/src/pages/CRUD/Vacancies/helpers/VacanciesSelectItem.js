...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vacancies/autocomplete`, { params });
  return response.data;
}

const VacanciesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vacancies.hasPermissionToCreate,
});

export default connect(select)(VacanciesSelectItem);
