...
import * as dataFormat from 'pages/CRUD/Vacancies/table/VacanciesDataFormatters';

// eslint-disable-next-line
import * as usersDataFormat from 'pages/CRUD/Users/table/UsersDataFormatters';

import actions from 'actions/vacancies/vacanciesListActions';
...
const VacanciesTable = () => {
  const [filters, setFilters] = React.useState([
    ...
    { label: 'Salary From', title: 'salary_from' },
    { label: 'Salary to', title: 'salary_to' },
    { label: 'Interest', title: 'interest', number: 'true' },
  ...
  const loading = useSelector((store) => store.vacancies.list.loading);
  const rows = useSelector((store) => store.vacancies.list.rows);
  const count = useSelector((store) => store.vacancies.list.count);
  const modalOpen = useSelector((store) => store.vacancies.list.modalOpen);
  const idToDelete = useSelector((store) => store.vacancies.list.idToDelete);
      ...
      <div>
        {null && (
          <Button
            color='default'
            size='xs'
            onClick={() => dispatch(push(`/admin/vacancies/${cell}`))}
          >
            View
          </Button>
        )}
        <Button
          color='info'
          size='xs'
          onClick={() => history.push(`/admin/vacancies/${cell}/edit`)}
        >
          Edit
        </Button>
        &nbsp;&nbsp;
        <Button
          color='danger'
          size='xs'
          onClick={(event) => openModal(event, cell)}
        >
          Delete
        </Button>
      </div>
  ...
  return (
    <div>
      <Widget title={<h4>Vacancies</h4>} collapse close>
        ...
        <ToolkitProvider columns={columns} data={rows} keyField='id' search>
          {(props) => (
            <>
              <SearchBar
                className='react-bootstrap-table-search-input'
                {...props.searchProps}
              />
              <BootstrapTable
                bordered={false}
                classes={`table-striped table-hover mt-4 ${
                  width < 1150 ? 'table-responsive' : ''
                }`}
