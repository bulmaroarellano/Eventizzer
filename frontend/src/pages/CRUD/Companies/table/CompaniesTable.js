...
import * as dataFormat from 'pages/CRUD/Companies/table/CompaniesDataFormatters';

import actions from 'actions/companies/companiesListActions';
...
const CompaniesTable = () => {
  ...
  const loading = useSelector((store) => store.companies.list.loading);
  const rows = useSelector((store) => store.companies.list.rows);
  const count = useSelector((store) => store.companies.list.count);
  const modalOpen = useSelector((store) => store.companies.list.modalOpen);
  const idToDelete = useSelector((store) => store.companies.list.idToDelete);
  ...
  <div>
    {null && (
    <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/companies/${cell}`))}>
    View
  </Button>
  )}
  <Button color="info" size="xs" onClick={() => history.push(`/admin/companies/${cell}/edit`)}>
  Edit
</Button>
  &nbsp;&nbsp;
...
<a href="#" className="page-link" onClick={handleClick}>{page}</a>
        ...
        return (
<div>
<Widget title={<h4>Companies</h4>} collapse close>
    <div>
        <a
            href={
                process.env.NODE_ENV === 'production'
                    ? window.location.origin + '/api-docs/#/Companies'
                    : 'http://localhost:8080/api-docs/#/Companies'
            }
        >
